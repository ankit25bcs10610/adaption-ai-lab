...
Usage:
  python -m src.fill_model_card --username myuser \
...
def main() -> None:
    ...
    if not base:
        print("  (note: baseline.json missing — run src.baseline first for real numbers)")
    if not ft:
        print("  (note: finetuned eval missing — run src.eval_harness first)")
