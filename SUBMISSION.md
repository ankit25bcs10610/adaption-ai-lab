...
| 3 | Build dataset with **Adaptive Data** | ✅ done | ran `datasets.run()`; **+15.7% quality, grade C→B** on the fixed set (`c4923b7f`, graded on 1,000/2,440 rows under the free-tier cap — full-set grade completes on the console run; the completed 250-row `a99c0c96` run corroborates at +10%, grade B); enhanced dataset downloaded to `data/adaptive_out/` |
