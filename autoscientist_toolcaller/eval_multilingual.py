...
The multilingual slice (`autoscientist_toolcaller/multilingual.py`) emits matched twins: the SAME tools + gold envelope with
...
Pure/offline (stdlib + the project's own eval helpers). CLI:
  python -m autoscientist_toolcaller.eval_multilingual --model <id> [--adapter path] --data data/out/test.jsonl \
